...
-(Category *)order{
    if (self.category.orderList==nil) {
        self.category.orderList=[[[NSMutableArray alloc] init] autorelease];
    }
    for (Recipe *aRecipe in self.category.orderList) {
        if (aRecipe==self) {
            return nil;
        }
    }
    [self.category.orderList addObject:self];
